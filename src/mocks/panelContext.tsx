...
interface MockPanelProviderProps {
  ...
  slices?: Record<string, { data: unknown; loading: boolean; error: Error | null }>;
  contextOverrides?: Partial<PanelContextValue>;
  actionsOverrides?: Partial<PanelActions>;
}

export const MockPanelProvider: React.FC<MockPanelProviderProps> = ({
  children,
  repository,
  slices,
  contextOverrides,
  ...
  // Build context overrides from props
  const finalContextOverrides: Partial<PanelContextValue> = {
    ...contextOverrides,
  };

  if (repository !== undefined) {
    finalContextOverrides.currentScope = {
      type: 'repository',
  ...
  if (slices) {
    const mockSlicesMap = new Map<string, DataSlice>();
    Object.entries(slices).forEach(([name, slice]) => {
      mockSlicesMap.set(
        name,
        createMockSlice(name, slice.data, 'repository')
      );
      if (slice.loading !== undefined) {
        mockSlicesMap.get(name)!.loading = slice.loading;
      }
      if (slice.error !== undefined) {
        mockSlicesMap.get(name)!.error = slice.error;
      }
    });
    finalContextOverrides.slices = mockSlicesMap;
    finalContextOverrides.getSlice = <T,>(name: string) =>
      mockSlicesMap.get(name) as DataSlice<T> | undefined;
    finalContextOverrides.hasSlice = (name: string) => mockSlicesMap.has(name);
    finalContextOverrides.isSliceLoading = (name: string) =>
      mockSlicesMap.get(name)?.loading ?? false;
  }

  const context = createMockContext(finalContextOverrides);
